...
def chat_history(request):
    ...
    return JsonResponse({'grouped_chats': sorted_grouped_chats})
...
def signupuser(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            user.save()
            login(request, user)
            return redirect('chatPage')  # Redirect after successful signup and login
    else:
        form = SignupForm()
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if contactus_instance.image:
                email.attach(contactus_instance.image.name, contactus_instance.image.read(),
                             contactus_instance.image.content_type)
            try:
                email.send()
                return redirect('contactus_success')  # Redirect after successful submission
